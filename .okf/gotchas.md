...
- **Reflection describes less than it serializes:** the wire handles more shapes than the descriptor generator maps. `DateTime`/`TimeSpan`/`decimal`/`Guid`, dictionary members and nested command types are skipped with a warning (`CommandNotDescribable`) — those handlers still execute, they're just not listed. Only field *numbering* is shared with the marshaller's model; the CLR→proto type mapping is separate.
