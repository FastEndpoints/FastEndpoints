...
- **OpenAPI schema `$ref` walking (Microsoft.OpenApi 2.x):** `OpenApiSchemaReference` often has null `Properties`/`Type` until resolved. Always `ResolveSchema()` (or export `ResolveForExport` + document `Components.Schemas` fallback when `HostDocument` is unset) before reading properties/composition — e.g. `.http` export `BuildPlaceholder` and request-example sample builders. Path-scoped cycle sets must key on the **resolved** schema so dual sibling refs to the same component both expand.
