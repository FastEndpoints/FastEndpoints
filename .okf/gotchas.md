...
- **Reflection describes less than it serializes:** the wire handles more shapes than the descriptor generator maps. `DateTime`/`DateOnly`/`TimeOnly`/`TimeSpan`/`decimal`/`Guid`/`Uri`, dictionary members and nested command types are skipped with a warning (`CommandNotDescribable`) — those handlers still execute (via protobuf-net inbuilts), they're just not listed. Only field *numbering* is shared with the marshaller's model; the CLR→proto type mapping is separate.
- **Protobuf BCL denylist:** `ProtobufMarshallerFactory.IsMessage` must keep known BCL specials out of attribute-free `Model.Add` (empty contracts → wire `0A00` / `default`). Supported non-messages that still round-trip: primitives, enums, `string`, `byte[]`, `decimal`, `DateTime`, `DateOnly`, `TimeOnly`, `TimeSpan`, `Guid`, `Uri`. Unsupported (fail at `Create`/`Register`, never silent-drop): `DateTimeOffset`, `Half`, `Int128`, `UInt128`, `Version`, `object`. Descriptor path uses the same list so these are never published as empty nested messages.
