...
- **Versioning strategies:** `DocumentVersionFilterTests` (unit: MaxEndpointVersion latest-in-range per bare route, range membership, filter+version composition) and `VersioningStrategyTests` (integration path asserts for release-group docs, ad hoc EndpointFilter + default max excluding `Version(1)`, release-version StartingRelease). Complements golden `release-*.json` / `release-versioning-*.json` snapshots.
