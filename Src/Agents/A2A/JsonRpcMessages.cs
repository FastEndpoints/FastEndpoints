...
sealed class JsonRpcError
{
    ...
    public static JsonRpcError InvalidRequest(string msg)
        => new() { Code = -32600, Message = msg };

    public static JsonRpcError InvalidParams(string msg)
        => new() { Code = -32602, Message = msg };

    public static JsonRpcError Internal(string msg, object? data = null)
        => new() { Code = -32603, Message = msg, Data = data };

    public static JsonRpcError TaskNotFound(string taskId)
        => new() { Code = -32001, Message = $"Task not found: {taskId}" };

    public static JsonRpcError ContentTypeNotSupported(string mediaType)
        => new() { Code = -32005, Message = $"Content type not supported: {mediaType}" };

    public static JsonRpcError VersionNotSupported(string version)
        => new() { Code = -32009, Message = $"A2A version not supported: {version}" };
}
...
sealed class A2APart
{
    ...
    [JsonPropertyName("mediaType"), JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
