...
{
    /// <summary>
    /// return <see langword="true" /> if this storage provider is used in a distributed scenario (multiple worker instances sharing the same data store).
    /// when enabled, job queues will always poll the storage periodically to discover jobs added by other workers.
    /// when disabled, polling only begins after the first job is queued or triggered on this instance.
    /// </summary>
    bool DistributedJobProcessingEnabled { get; }
    ...
    /// <para>
    /// in distributed scenarios (multiple worker instances sharing the same data store), this method must use a database-level atomic operation
    /// (e.g., SQL <c>UPDATE...OUTPUT</c> with <c>READPAST</c>, PostgreSQL <c>UPDATE...RETURNING</c> with
    ...
    /// </para>
    /// <para>
    /// in distributed scenarios, you should also reset the <see cref="IJobStorageRecord.DequeueAfter" /> property
    /// (e.g., to <see cref="DateTime.MinValue" /> or to the same value as <see cref="IJobStorageRecord.ExecuteAfter" />)
    /// so that the job becomes eligible for any worker to pick up again.
    /// </para>
    /// </summary>
