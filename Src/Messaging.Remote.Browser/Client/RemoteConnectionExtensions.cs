...
public static class RemoteConnectionExtensions
{
    ...
    public static IServiceProvider MapRemote(this IServiceProvider provider, string remoteAddress, Action<RemoteConnection> r)
    {
        r(new(remoteAddress, provider));
        var logger = provider.GetRequiredService<ILogger<RemoteConnection>>();
        logger.RemoteConfigured(remoteAddress, RemoteConnection.RemoteMap.Count);
