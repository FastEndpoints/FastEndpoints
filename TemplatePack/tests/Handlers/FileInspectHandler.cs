    ...
    public class FileInspectHandler : IHandler
    {
            ...
            JObject json)
        {
            ...
            try {
                ...
                string _ = tokens["targetPathName"];
                    ...
                    foreach (var expectation in expectations.Children().OfType<JObject>()) {
                        ...
                        switch (assertion) {
                            case "EXISTS":
                                if (!doesFileExist) {
                                    return new ExecuteHandlerResult(watch.Elapsed, false,
                                        $"Expected file \"{filename}\" to exist, but it did not", name: name);
                                }
                            ...
                            case "DOES_NOT_EXIST":
                                if (doesFileExist) {
                                    return new ExecuteHandlerResult(watch.Elapsed, false,
                                        $"Expected file \"{filename}\" to not exist, but it did", name: name);
                                }

                                break;
                            case "CONTAINS":
                                text = expectation["text"]?.Value<string>();
                                if (!Enum.TryParse(expectation["comparison"]?.Value<string>() ?? "OrdinalIgnoreCase",
                                    out comparison)) {
                                ...
                                if (!doesFileExist) {
                                    return new ExecuteHandlerResult(watch.Elapsed, false,
                                        $"Expected file \"{filename}\" to contain \"{text}\" ({comparison}), but file did not exist",
                                        name: name);
                                }
                                ...
                                if (fileContent.IndexOf(text, comparison) < 0) {
                                    return new ExecuteHandlerResult(watch.Elapsed, false,
                                        $"Expected file \"{filename}\" to contain \"{text}\" ({comparison}), but it did not",
                                        name: name);
                                }
                            ...
                            case "DOES_NOT_CONTAIN":
                                ...
                                if (!doesFileExist) {
                                    return new ExecuteHandlerResult(watch.Elapsed, false,
                                        $"Expected file \"{filename}\" to not contain \"{text}\" ({comparison}), but file did not exist",
                                        name: name);
                                }
                                ...
                                if (fileContent.IndexOf(text, comparison) >= 0) {
                                    return new ExecuteHandlerResult(watch.Elapsed, false,
                                        $"Expected file \"{filename}\" to not contain \"{text}\" ({comparison}), but it did",
                                        name: name);
                                }
