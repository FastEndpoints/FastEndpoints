...
namespace ProjectTestRunner.Handlers
{
    public class HttpRequestHandler : IHandler
    {
            ...
            JObject json)
        {
            ...
            try {
                ...
                try {
                            ...
                            switch (assertion) {
                                case "RESPONSE_CONTAINS":
                                    ...
                                    if (responseText.IndexOf(s, c) < 0) {
                                        return new ExecuteHandlerResult(watch.Elapsed, false,
                                            $"Expected output to contain \"{s}\" ({c}), but it did not", name: name);
                                    }
                                ...
                                case "RESPONSE_DOES_NOT_CONTAIN":
                                    ...
                                    if (responseText.IndexOf(s, c) > -1) {
                                        return new ExecuteHandlerResult(watch.Elapsed, false,
                                            $"Expected output to NOT contain \"{s}\" ({c}), but it did", name: name);
                                    }
                                ...
                                case "RESPONSE_HEADER_CONTAINS":
                                    ...
                                    if (!response.Headers.TryGetValues(key, out values)) {
                                        return new ExecuteHandlerResult(watch.Elapsed, false,
                                            $"Expected a response header called \"{key}\" to be present, but it was not",
                                            name: name);
                                    }

                                    if (!values.Any(x => x.IndexOf(s, c) > -1)) {
                                        return new ExecuteHandlerResult(watch.Elapsed, false,
                                            $"Expected a response header called \"{key}\" to have a value \"{s}\", but it did not",
                                            name: name);
                                    }
                                ...
                                case "RESPONSE_HEADER_DOES_NOT_CONTAIN":
                                    ...
                                    if (values.Any(x => x.IndexOf(s, c) > -1)) {
                                        return new ExecuteHandlerResult(watch.Elapsed, false,
                                            $"Expected a response header called \"{key}\" to NOT have a value \"{s}\", but it did",
                                            name: name);
                                    }
                                ...
                                case "DOES_NOT_HAVE_HEADER":
                                    ...
                                    if (response.Headers.TryGetValues(key, out values)) {
                                        return new ExecuteHandlerResult(watch.Elapsed, false,
                                            $"Expected a response header called \"{key}\" to NOT be present, but it was",
                                            name: name);
                                    }
                    ...
                    return new GenericHandlerResult(watch.Elapsed, success,
                        success ? null : $"Expected {status} but got {response.StatusCode}");
                }
                catch (Exception ex) {
        ...
        public string Summarize(IReadOnlyDictionary<string, string> tokens, JObject json)
        {
            ...
            return
                $"Web Request - {verb} {url} (Body? {body != null}, Encoding? {requestEncoding}, MediaType? {requestMediaType}) -> Expect {status}";
        }
    }
}
